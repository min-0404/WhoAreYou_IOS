    ...
    var body: some View {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 28) {
                    ...
                    VStack(spacing: 12) {
                        ...
                        InputField(icon: "circle.grid.cross.fill", placeholder: "OTP 번호를 입력하세요", text: $otp, isSecure: false)
                        InputField(icon: "lock.circle.fill", placeholder: "새 비밀번호를 입력하세요", text: $newPassword, isSecure: true)
                        InputField(icon: "lock.rotation", placeholder: "비밀번호를 재입력하세요", text: $confirmPassword, isSecure: true)
        ...
        .alert("알림", isPresented: $showAlert) {
            Button("확인", role: .cancel) {
                if alertMessage.contains("완료") { dismiss() }
            }
        } message: {
            Text(alertMessage)
        }
